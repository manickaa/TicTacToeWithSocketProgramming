...
# Summer 2020
# Program: server.py
# References: https://pythonprogramming.net/server-chatroom-sockets-tutorial-python-3/
...
def init_game(client_socket):
	...
	print_board(board)	#function call to print the intialized board
	print("Type /q when you want to quit")
	start_game(client_socket, board)	#start the game
...
def start_game(client_socket, board):
	...
	while not isBoardFull(board):
		...
		if(str(server_move) == '/q'):
			...
			client_socket.send(bytes(server_quit, 'utf-8'))
			break
		...
		if (isBoardFull(board) and  not checkWinner(board, 'X')):
			...
			print(match_draw)
			client_socket.send(bytes(match_draw, 'utf-8'))
			client_socket.send(bytes(str(server_move), 'utf-8'))
			break

		#check if server wins
		if checkWinner(board, 'X'):
			print("Server wins this time")
			server_win = "Server Won!!! You lose...See you next time :) "
			client_socket.send(bytes(server_win, 'utf-8'))
			client_socket.send(bytes(str(server_move), 'utf-8'))
			break
		...
		#If the server has not won yet and the board is not full
			...
			client_move = client_socket.recv(1024).decode('utf-8')
		...
		if checkWinner(board, 'O'):
			...
			client_win = "Hurrayy...You Won!!! See you next time :) "
			client_socket.send(bytes(client_win, 'utf-8'))
			break
...
def client_chatroom(server_socket):
	...
	if(re.search("yes",msg_recv)):
		...
		client_socket.send(bytes(goodbye_msg, 'utf-8'))
		return
...
def main():
	...
	#call for chatroom function
	client_chatroom(server_socket)
